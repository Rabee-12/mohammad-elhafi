...
/* ============================================================
   CONTACT — fill these in to enable the WhatsApp / phone buttons.
   They stay hidden until filled, so nothing looks broken.
   ============================================================ */
let CONTACT = {};
...
const ANALYTICS = {
  ...
  plausible: "",   // Plausible domain, e.g. "bhaleeb.com"
};

/* ---------- build flat lookup + basket state ---------- */
const FLAT = {};
function indexData(){ MENU.forEach(cat=>cat.items.forEach((it,idx)=>{ it._id = cat.id+"-"+idx; it._slug = cat.slug; FLAT[it._id] = it; })); }
let basket = {};
try{ basket = JSON.parse(localStorage.getItem("bhaleeb_basket") || "{}"); }catch(e){ basket = {}; }
const saveBasket = ()=> localStorage.setItem("bhaleeb_basket", JSON.stringify(basket));
...
const I18N = {
  tr:{ dir:"ltr", scroll:"menüye kaydır", price:"Fiyatlar ₺ (TL) cinsindendir", made:"💙 ile sevgiyle yapıldı",
       ...
       soldout:"Tükendi", waOrder:"Siparişi WhatsApp'tan gönder", waHello:"Merhaba! Siparişim:", soon:"Yakında", location:"Konum" },
  en:{ dir:"ltr", scroll:"scroll to menu", price:"Prices in Turkish Lira (₺)", made:"Made with 💙 & milk",
       ...
       soldout:"Sold out", waOrder:"Send order on WhatsApp", waHello:"Hello! My order:", soon:"Soon", location:"Location" },
  ar:{ dir:"rtl", scroll:"اسحب لرؤية القائمة", price:"الأسعار بالليرة التركية (₺)", made:"صُنع بكل 💙 وحليب",
       receipt:"حسابي", total:"المجموع", note:"هذا مجرد ملخص للحساب — لا يوجد طلب أو دفع.", clear:"مسح", empty:"لم تُضف أي صنف بعد", call:"اتصل",
       soldout:"نفذ", waOrder:"أرسل الطلب عبر واتساب", waHello:"مرحبا! هذا طلبي:", soon:"قريباً", location:"الموقع" },
};
...
let LANG = localStorage.getItem("bhaleeb_lang") || "tr";
...
function openItem(id){
  ...
  document.getElementById("itemDesc").textContent  = FLAT[id].desc ? (FLAT[id].desc[LANG] || "") : "";
...
function setLang(l){
  if(!I18N[l]) return;
  LANG = l;
  localStorage.setItem("bhaleeb_lang", l);
  updateLangUI(l);
  applyStatic();
  render();
...
/* language dropdown */
const LANG_CODE = { tr:"TR", en:"EN", ar:"ع" };
...
(function heroStrip(){
  ...
  if(!m) return;
  const picks = [
    "bhaleeb/1.jpg","trilece/10.jpg","kashri/1.jpg","slush/13.jpg","milkshake/3.jpg",
    "waffle/3_1.jpg","juice/2.jpg","icecream/4.jpg","trilece/6.jpg","kadayif/1.jpg",
    "bhaleeb/3.jpg","slush/1.jpg"
  ];
...
fetch("data/menu.json?v=" + Date.now(), { cache: "no-store" })   // ?v= busts any browser/server cache
  .then(r => r.json())
  .then(d => { MENU = d.categories || []; CONTACT = Object.assign(CONTACT, d.contact || {}); boot(); })
  .catch(e => { console.error(e); document.getElementById("menu").innerHTML =
    "<p style='padding:2rem;text-align:center;color:#0b3d91'>Menü yüklenemedi · Menu failed to load</p>"; });
