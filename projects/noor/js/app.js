...
(() => {
  ...
  function applyLang() {
    ...
    document.title = document.body.dataset.title
      ? `${document.body.dataset.title} — NOOR نور`
      : "NOOR نور — " + ({ en: "Modest Couture", tr: "Tesettür Couture", ar: "أزياء محتشمة راقية" }[lang]);
  }
  ...
  window.setLang = (l) => {
    ...
    applyLang();
    document.dispatchEvent(new CustomEvent("noor:lang"));
  };
  ...
  window.NOOR = { t, fmt, img, get lang() { return lang; } };

  /* ---------- cart ---------- */
  let cart = [];
  try { cart = JSON.parse(localStorage.getItem("noor_cart")) || []; } catch (e) {}

  const saveCart = () => localStorage.setItem("noor_cart", JSON.stringify(cart));
  ...
  window.productCard = (p, revealDelay = 0) => `
    <a class="p-card rv ${revealDelay ? "rv-d" + revealDelay : ""}" href="product.html?id=${p.id}">
      <div class="p-media">
        ${p.tag ? `<span class="p-tag">${t("tag_" + p.tag)}</span>` : ""}
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    renderCart();
    updateBadge();
    document.dispatchEvent(new CustomEvent("noor:ready"));
    watchReveals();
  });

  document.addEventListener("noor:lang", () => { renderCart(); });
})();
